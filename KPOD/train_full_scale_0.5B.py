...
def main():
    ...
    config.model.student_model_name = "Qwen/Qwen2-0.5B"   # or "Qwen/Qwen1.5-4B"

    # Dataset
    config.data.dataset_name = "gsm8k"

    # Training hyperparameters for 4B model
    config.training.batch_size = 8                # small model → much larger batch fits
    config.training.gradient_accumulation = 2     # effective batch size = 16
    config.training.lr_flan = 5e-5                # 5× higher learning rate
    config.training.max_grad_norm = 1.0           # small models tolerate more
    config.training.warmup_ratio = 0.1            # slightly longer warmup helps stability
    config.training.weight_decay = 0.05           # 0.1 is too strong for tiny LMs
    config.training.num_epochs = 100              # still allowed
